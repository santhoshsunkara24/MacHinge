...
fragment float4 duoFoldFragmentShader(
    ...
    // Blur radius grows with fold progress p + dynamic velocity
    float pGlass = pow(p, 0.80);
    float baseRadius = 0.0075 * pGlass * min(uniforms.blurIntensity, 1.8);
    ...
    // =========================================================================
    // Smooth, progressive timing curve:
    // - Micro-movements (p < 0.12): zero glow, display remains completely normal
    // - 20-25% closing fold: soft, smooth awakening starts becoming visible
    // - Intermediate fold (p ~ 0.4-0.7): rich cascading light sweep
    // - Deep fold / near closed (p -> 1.0): high-intensity dazzling pure white radiance
